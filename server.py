...
try:
    ...
    print(str(e))
    print("[SERVER] UDP-Server could not start")
    quit()

S.listen()  # TCP listen for connections
...
def threaded_client(conn, _id):
	...
	try:
		...
		print(f"[ERROR] Failed to receive name from client {_id}: {e}")
		conn.close()
		return
	...
	try:
		...
		print(f"[ERROR] Failed to send initial data to client {_id}: {e}")
	...
	while True:
		...
		try:
			...
			print(f"[ERROR] Error handling data from client {_id}: {e}")
			break
...
def broadcast():
	try:
		UDPS.sendto('Welcome to Agar.io!!!'.encode(), (UDP_IP, UDP_PORT))
	except Exception as e:
